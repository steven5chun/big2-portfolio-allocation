...
class Card:
    ...
    def __eq__(self, other):
        return isinstance(other, tuple) and self.value == other[0] and self.suit == other[1] or \
               isinstance(other, Card) and self.value == other.value and self.suit == other.suit
...
class Deck:
    ...
    def get_random_cards(self, n):
        array_of_index = random.sample(range(0, len(self.cards)), n)
        array_of_index.sort()
        cards = [self.cards[i] for i in array_of_index]
        for i in reversed(array_of_index):
            self.cards.pop(i)
        return cards
...
class Player:
    ...
    def full_house_combinations(self):
        all_full_houses = []
        temp = self.hand_to_int()
        three_ranks = [r for r in range(3, 16) if temp.count(r) >= 3]
        pair_ranks = [r for r in range(3, 16) if temp.count(r) >= 2]
        for t_rank in three_ranks:
            for p_rank in pair_ranks:
                if t_rank == p_rank:
                    continue
    ...
    def two_pair_combinations(self):
        temp = self.hand_to_int()
        all_two_pairs = []
        pair_ranks = [r for r in range(3, 16) if temp.count(r) >= 2]
        for i_idx in range(len(pair_ranks)):
            for j_idx in range(i_idx + 1, len(pair_ranks)):
                r1 = pair_ranks[i_idx]
                r2 = pair_ranks[j_idx]
    ...
    def print_two_pair_combinations(self):
        two_pairs = self.two_pair_combinations()
        print(f"Found {len(two_pairs)} possible two pairs:")
        for i, two_pair in enumerate(two_pairs, 1):
            print(f"Two pair {i}: {' '.join(str(card) for card in two_pair)}")
    ...
    def number_of_combinations_flush(self):
        ...
        count = 0
        for suit in suits:
            suit_count = sum(1 for _, j in self.hand if j == suit)
            if suit_count >= 5:
                count += comb(suit_count, 5)
        return count
    ...
    def number_of_combinations_two_pair(self):
        temp = self.hand_to_int()
        pair_ranks = [(r, temp.count(r)) for r in range(3, 16) if temp.count(r) >= 2]
        count = 0
        for i in range(len(pair_ranks)):
            for j in range(i + 1, len(pair_ranks)):
                count += comb(pair_ranks[i][1], 2) * comb(pair_ranks[j][1], 2)
        return count
...
def main():
    ...
    print("Exist two pair:" + str(player.exist_two_pair()))
    ...
    print("Combinations of two pair: " + str(player.number_of_combinations_two_pair()))
    ...
    player.print_two_pair_combinations()
